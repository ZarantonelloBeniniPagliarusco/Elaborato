...
/**
 *
 * Questa classe serve a controllare dove il giocatore pu� muovere
 */
...
public class MoveChecker {
	...
	if(Board.casella[rig][col].getPlayer() != 0) { //se la casella selezionata contiene una pedina & le sottostanti sono vuote
		...
		for(int i = 0; i < 8; i++) 
			...
			if(Board.casella[i][j].getBackground() == Board.green) {
			    ...
			    Board.text.setText("Pi� di una mangiata disponibile. Scegline una.");
			    x=1;
			    select(rig,col);
	...
	if((rig-1)>=0 && (col-1)>=0){
	    if(Board.casella[rig][col].getPlayer() == 2 && Board.casella[rig-1][col-1].getPlayer() == 0) { //caso 2: sopra a sx � libero
	...
	if ((rig-1) >= 0 && (col+1) <= 7) {
	    ...
	    if(Board.casella[rig][col].getPlayer() == 2 && Board.casella[rig-1][col+1].getPlayer() == 0) { //caso 3: sopra a dx � libero
		Board.casella[rig-1][col+1].select("yellow");
	...
	if((rig-1)>=0 && (col-1)>=0)
	    if(Board.casella[rig-1][col-1].getPlayer() == 0) { //caso 2: sopra a sx � libero
		Board.casella[rig-1][col-1].select("yellow");
		Board.selected = false;
	    }

	if ((rig-1) >= 0 && (col+1) <= 7) 		
	    if(Board.casella[rig-1][col+1].getPlayer() == 0) { //caso 3: sopra a dx � libero
		Board.casella[rig-1][col+1].select("yellow");
		Board.selected = false;
	...
	/**
	 * Controllo se ci sono altre caselle gi� selezionate, in quel caso le deseleziono
	 * @param nessuno
	 */
